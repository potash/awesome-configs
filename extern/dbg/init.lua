...
local dumper     = require("extern.dbg.dumper")
...
local function dump(data)
    ...
    color["thread"]="#a02f5f"
    color["table"]="#0049BA"
    local text = "<b><span color='"..color[type(data)].."'>"..type(data).."</span></b>: "..dumper(data)
...
function module.error(msg)
    local text
    if not msg then
        text = "A call into the lua code aborted with an error"
    else
        text = msg
    end
    naughty.notify({
